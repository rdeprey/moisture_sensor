...
const getSensorReadings = (sensor) => {
  return new Promise((resolve, reject) => {
    sensor.read((readError, reading) => {
      if (readError) {
        return reject(
          new Error(`There was an error getting the sensor reading:
                    ${readError}`)
        );
      }
...
const getMoistureLevel = () => {
  ...
  return new Promise((resolve, reject) => {
    const sensor = mcpadc.open(5, { speedHz: 20000 }, (error) => {
      if (error) {
        return reject(
          new Error(`There was an error accessing the sensor: ${error}`)
        );
      }
      ...
      return Promise.all(readingPromises).then(() => {
        const averageRawValue =
          readings.rawValues.reduce((a, b) => a + b, 0) / 100;
        ...
        return resolve({
          ...
          soilDrynessPercentage:
            averageRawValue > 0
              ? (((averageRawValue - valueRange) / valueRange) * 100).toFixed(0)
              : 0
        });
      });
    });
...
const getWateringStatus = () => {
  return new Promise((resolve, reject) => {
    const pumpRelay = i2c.open(1, (err) => {
      if (err) {
        return reject(
          new Error(`There was an error opening the pump relay: ${err}`)
        );
      }
      ...
      pumpRelay.readWord(relayAddress, 0x04, (err, rawData) => {
        if (err) {
          return reject(
            new Error(
              `There was an error getting the pump relay status: ${err}`
            )
          );
        }
      ...
      pumpRelay.readWord(relayAddress, 0x04, async (err, rawData) => {
        ...
        if (rawData === 0 && needsWater) {
          ...
          pumpRelay.writeWord(relayAddress, 0x04, 0xff, (err, data) => {
            if (err) {
              return reject(
                new Error(`There was an error starting the pump relay: ${err}`)
              );
            }
      ...
      pumpRelay.readWord(relayAddress, 0x04, (err, rawData) => {
        ...
        if (rawData > 0) {
          pumpRelay.writeWord(relayAddress, 0x04, 0x00, (err, data) => {
            if (err) {
              return reject(
                new Error(`There was an error stopping the pump relay: ${err}`)
              );
            }
          });
        }
  ...
  return getMoistureLevel()
    .then((level) => {
      if (level.soilDrynessPercentage < 45) {
        return resolve(
          `Yes, the plants could use watering. The current moisture level is ${level.soilDrynessPercentage}%.`
        );
      } else {
        return resolve(
          `No, the plants are doing well. The current moisture level is ${level.soilDrynessPercentage}%.`
        );
      }
    })
    .catch((error) => {
